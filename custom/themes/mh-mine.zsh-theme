...
# DONE add precmd hook to only print last exit status once, see apple.zsh-theme
...
theme_precmd() {
  if $theme_ran_something; then
    return_code="%(?..%{$fg[red]%}%? ↵%{$reset_color%})"
  else
    return_code=""
  fi
    theme_ran_something=false
}
theme_preexec() {
    theme_ran_something=true
}
